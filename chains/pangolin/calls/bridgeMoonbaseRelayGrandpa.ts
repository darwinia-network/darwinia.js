...
export const getBridgeMoonbaseRelayGrandpa = (dispatch: Dispatch, metadata: Metadata) => {
    return {
        ...
        setOperational: async (signer: ethers.Signer, _operational: unknown): Promise<ethers.providers.TransactionReceipt> => {
            return await dispatch(signer, 'BridgeMoonbaseRelayGrandpa', 'setOperational', false, {
                operational: _operational,
	    });
        },

        setOperationalH: async (signer: ethers.Signer, argsBytes: BytesLike): Promise<ethers.providers.TransactionReceipt> => {
            return await dispatch(signer, 'BridgeMoonbaseRelayGrandpa', 'setOperational', true, argsBytes);
        },
