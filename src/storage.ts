...
export function getStorage(provider: BaseProvider, metadata: Metadata) {
  return async (prefix: string, method: string, ...input: Array<unknown>): Promise<string | null> => {
    ...
    if (raw.toString() == "0x") {
      ...
      const type = metadata.registry.lookup.getTypeDef(valueType).type
      const typeString = type.includes('"') ? `{"result": ${type}}` : `{"result": "${type}"}`
      let result = metadata.registry.createTypeUnsafe(typeString, [raw]);
      const decoded = (result.toJSON() as { result: any })['result']
